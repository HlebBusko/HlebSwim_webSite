...
function Footer(){
  ...
  const links = [{title: 'About me', id: 'about'}, {title: 'Offer', id: 'offer'}, {title: 'Contact', id: 'contact'}]
    ...
    <div className=" flex justify-between lg:px-16 md:px-8 px-4 py-2">
      <div>
        ...
        <ul className="flex flex-col">
          {links.map((link) => <Link className="text-footerText text-sm hover:text-main cursor-pointer" smooth={true} duration={300} to={link.id} offset={-70}  key={link.id}> {link.title}</Link>)}
        </ul>
      </div>
      <div>
