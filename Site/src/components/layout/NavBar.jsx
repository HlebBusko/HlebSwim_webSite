...
function NavBar() {
  ...
  return (
    <div ref={dropDownRef}>
      ...
      >
          ...
          <ul className="lg:gap-5 gap-3 items-center h-full hidden sm:flex">
            {links.map((link) => (
              <NavLink
                ...
                link={link}
                className={
                  'h-full cursor-pointer hover:text-main flex px-2 items-center rounded w-full transition-all duration-300 font-medium text-blackText whitespace-nowrap'
                }
                offset={-70}
                activeClass={
        ...
        <div className="right-section flex items-center ml-6 gap-3">
          ...
          <Link
            ...
            offset={-70}
          >
            <ButtonMain className="bg-main text-white hover:bg-white border-main hover:text-main shadow-lg">
              Enroll
            </ButtonMain>
          </Link>
        </div>
      ...
      <ul
        className={`fixed top-18 ${isDisplayed ? 'translate-y-0' : '-translate-y-60'}  w-full transition-all duration-700 ease-in-out z-40 shadow`}
      >
        {links.map((link) => (
          <NavLink
            key={link.id}
            link={link}
            className={
              'w-full flex align-center justify-center py-2 transition-all ease-in-out duration-400 bg-white hover:text-white hover:bg-main font-medium cursor-pointer'
            }
            offset={-80}
